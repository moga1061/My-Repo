...
addon_id = 'plugin.video.jfoised'
plugin = xbmcaddon.Addon(id=addon_id)

DB = os.path.join(xbmc.translatePath("special://database"), 'jfoised.db')
BASE_URL = 'http://jfoised.net/'
net = Net()
addon = Addon('plugin.video.jfoised', sys.argv)
showAllParts = True
showPlayAll = True
...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        print 'jfoised get Movie Titles Menu %s' % url
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.jfoised/?mode=Search&query=%s)' %(name.strip())
        		cm.append(('Search on jfoised', runstring))
...
def MainMenu():    #homescreen
        addon.add_directory({'mode': 'Categories', 'section': 'movies'},  {'title':  '[COLOR blue]Movies> >[/COLOR]'}, img=IconPath + 'mo.png')
        addon.add_directory({'mode': 'Categories', 'section': 'tv'},  {'title':  '[COLOR blue]Tv Shows> >[/COLOR]'}, img=IconPath + 'tv.png')
...
def GetSearchQuery():
	...
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR green]Search release[/COLOR]')
...
def Search(query):
        url = 'http://www.google.com/search?q=site:jfoised.net ' + query
        url = url.replace(' ', '+')
        print url
