...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.theextopia/?mode=Search&query=%s)' %(name.strip())
...
def MainMenu():    #homescreen
        addon.add_directory({'mode': 'Categories', 'section': 'a'},  {'title':  '[COLOR blue]Anime >>>[/COLOR]'}, img=IconPath + 'anime.png')
        addon.add_directory({'mode': 'Categories', 'section': 'm'},  {'title':  '[COLOR blue]Movies>>>[/COLOR]'}, img=IconPath + 'movies.png')
        addon.add_directory({'mode': 'Categories', 'section': 't'},  {'title':  '[COLOR blue]TV shows>>>[/COLOR]'}, img=IconPath + 'tv.png')
...
def GetSearchQuery():
	...
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR green]Search Extopia[/COLOR]')
...
def Search(query):
        url = 'http://www.google.com/search?q=site:theextopia.com ' + query
        url = url.replace(' ', '+')
        print url
