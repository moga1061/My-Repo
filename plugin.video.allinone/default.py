...
def GetTitles12(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles #omp
        ...
        for page in range( start, end):
                if ( page != start):
                        ...
                        html = net.http_GET(pageUrl).content
                        CLEAN(html)                        
                match = re.compile('<p><a href="(.+?)"><img class=.+? alt="(.+?)" src="(.+?)" width=.+? height=.+? /></a></p>', re.DOTALL).findall(html)
                for movieUrl, name, img in match:
                        addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': movieUrl}, {'title':  name.strip()}, img= img, fanart=FanartPath + 'fanart.png')
...
def GetTitles12a(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles #omp2
        ...
        for page in range( start, end):
                if ( page != start):
                        ...
                        html = net.http_GET(pageUrl).content
                        CLEAN(html)                        
                match = re.compile('<h2 class=.+? id=.+?><a href="(.+?)" rel=.+? title=.+?>(.+?)</a></h2>', re.DOTALL).findall(html)
                for movieUrl, name in match:
...
def MovieMenu():   #movies 
        ...
        addon.add_directory({'mode': 'WtMenu'}, {'title':  '[COLOR goldenrod](HD) [/COLOR][COLOR lightsteelblue][B]Inter. Movies Genre[/B][/COLOR] [COLOR fuchsia](Moviesall4u/All4Youz) [/COLOR]>>'}, img=IconPath + 'maa.png', fanart=FanartPath + 'fanart.png')
...
def WtMenu():   #Moviesall4u  #all4youz
