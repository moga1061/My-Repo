...
addon_id = 'plugin.video.wttyid'
plugin = xbmcaddon.Addon(id=addon_id)

DB = os.path.join(xbmc.translatePath("special://database"), 'wttyid.db')
BASE_URL = 'http://watchthetapes.com/'
net = Net()
addon = Addon('plugin.video.wttyid', sys.argv)
showAllParts = True
showPlayAll = True
...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        print 'wttyid get Movie Titles Menu %s' % url
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.wttyid/?mode=Search&query=%s)' %(name.strip())
        		cm.append(('Search on wttyid', runstring))
                        addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': movieUrl}, {'title':  name.strip()}, contextmenu_items= cm, img= img)



                addon.add_directory({'mode': 'GetTitles', 'url': url, 'startPage': str(end), 'numOfPages': numOfPages}, {'title': '[COLOR salmon][B][I]Next page...[/B][/I][/COLOR]'}, img=IconPath + 'next.png')
...
def MainMenu():    #homescreen
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/movies/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR salmon]Latest Movies >>[/COLOR]'}, img=IconPath + 'la.png')
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/tvshows',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR salmon]Latest Tv shows >>[/COLOR]'}, img=IconPath + 'tv.png')
        ...
        addon.add_directory({'mode': 'ResolverSettings'}, {'title':  '[COLOR red]Resolver Settings[/COLOR]'}, img=IconPath + 'setings.png')
...
def GetSearchQuery():
	...
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR salmon]Search[/COLOR]')
