...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        ...
        for page in range( start, end):
                ...
                match = re.compile('suf-tile-topmost.+?href="(.+?)".+?src="(.+?)".+?href=.+?>(.+?)<', re.DOTALL).findall(html)
...
def MainMenu():    #homescreen
        addon.add_directory({'mode': 'Categories', 'section': 'movies'},  {'title':  '[COLOR blue]Movies >>[/COLOR]'}, img=IconPath + 'movie.png')
        addon.add_directory({'mode': 'Categories', 'section': 'tv-shows'},  {'title':  '[COLOR blue]Tv Shows >>[/COLOR]'}, img=IconPath + 'tv.png')
        addon.add_directory({'mode': 'GetSearchQuery'},  {'title':  '[COLOR green]Search Movies & Tv shows >>[/COLOR]'}, img=IconPath + 'search.png')
