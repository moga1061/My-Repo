...
addon_id = 'plugin.video.hdtv-release'
plugin = xbmcaddon.Addon(id=addon_id)

DB = os.path.join(xbmc.translatePath("special://database"), 'hdtv-release.db')
BASE_URL = 'http://hdtv-release.com/'
net = Net()
addon = Addon('plugin.video.hdtv-release', sys.argv)
showAllParts = True
showPlayAll = True
...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        print 'hdtv-release get Movie Titles Menu %s' % url
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.hdtv-release/?mode=Search&query=%s)' %(name.strip())
        		cm.append(('Search on hdtv-release', runstring))
...
def MainMenu():    #homescreen
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/hdtv/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue]Latest HDTV shows >>[/COLOR]'}, img=IconPath + 'tv.png')
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/hdtv/tv-pack/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  'Full HDTV series >>'}, img=IconPath + 'tv.png')
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/hdtv/hdtv-hdtv/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  'HDTV shows >>'}, img=IconPath + 'tv.png')
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/hdtv/web-dl/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  'Web-dl HDTV shows >>'}, img=IconPath + 'tv.png')
...
def GetSearchQuery():
	...
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR green]Search HDTV Release[/COLOR]')
...
def Search(query):
        url = 'http://www.google.com/search?q=site:hdtv-release.com ' + query
        url = url.replace(' ', '+')
        print url
