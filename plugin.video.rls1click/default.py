...
addon_id = 'plugin.video.rls1click'
plugin = xbmcaddon.Addon(id=addon_id)

DB = os.path.join(xbmc.translatePath("special://database"), 'rls1click.db')
BASE_URL = 'http://rls1click.com/'
net = Net()
addon = Addon('plugin.video.rls1click', sys.argv)
showAllParts = True
showPlayAll = True
...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        print 'rls1click get Movie Titles Menu %s' % url
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.rls1click/?mode=Search&query=%s)' %(name.strip())
        		cm.append(('Search on rls1click', runstring))
...
def GetSearchQuery():
	...
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR green]Search release[/COLOR][COLOR aqua][B]1 click[/B][/COLOR]')
...
def GetSearchQuery2():
	...
	if not last_search: last_search = ''
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR green]Scene Source Search[/COLOR]')
	keyboard.setDefault(last_search)
	keyboard.doModal()
	if (keyboard.isConfirmed()):
                query = keyboard.getText()
...
def GetSearchQuery3():
	...
	if not last_search: last_search = ''
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR green]OneClickWatch Search[/COLOR]')
	keyboard.setDefault(last_search)
	keyboard.doModal()
	if (keyboard.isConfirmed()):
                query = keyboard.getText()
...
def Search4(query):
        url = 'http://www.google.com/search?q=site:scenelog.eu ' + query
        url = url.replace(' ', '+')
        print url
        html = net.http_GET(url).content
        CLEAN(html)
