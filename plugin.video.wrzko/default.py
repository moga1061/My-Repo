...
addon_id = 'plugin.video.wrzko'
plugin = xbmcaddon.Addon(id=addon_id)

DB = os.path.join(xbmc.translatePath("special://database"), 'wrzko.db')
BASE_URL = 'http://www.wrzko.eu/'
net = Net()
addon = Addon('plugin.video.wrzko', sys.argv)
showAllParts = True
showPlayAll = True
...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): # Get Movie Titles
        print 'wrzko get Movie Titles Menu %s' % url
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.wrzko/?mode=Search&query=%s)' %(name.strip())
        		cm.append(('Search on wrzko', runstring))
...
def GetSearchQuery():
	...
	keyboard = xbmc.Keyboard()
        keyboard.setHeading('[COLOR green]Search wrzko[/COLOR]')
